...
def vision_loop(debug=False, record_dir=None):
    ...
    camera = LineCamera()
    try:
        modo_calibracao = camera.capture_mode_id
        calibracao_wide = carregar_calibracao(
            config.GREEN_WIDE_CALIBRATION_PATH,
            resolution=(camera_x, camera_y),
            camera_index=camera.camera_index,
            sensor=camera.sensor_id,
            mode=modo_calibracao,
        )
        lens_position = camera.aplicar_posicao_lente(
            calibracao_wide.metadata.lens_position)
        calibracao_wide.validate_compatibility(
            (camera_x, camera_y),
            camera_index=camera.camera_index,
            sensor=camera.sensor_id,
            capture_mode=modo_calibracao,
        ...
        print(
            "[visao] calibracao wide valida: "
            f"RMS={calibracao_wide.metadata.rms_px:.3f}px, "
            f"LensPosition={lens_position:.4f}, "
            f"{calibracao_wide.metadata.view_count} vistas"
        )
    ...
    try:
        while not terminate.value:
            ...
            direcao_verde_bruta = observacao_topologica.decision.name.lower()
            candidato_verde_frame = bool(
                observacao_topologica.markers
                or GreenDecision(int(observacao_topologica.decision))
                in (GreenDecision.PENDING, GreenDecision.LEFT,
                    GreenDecision.RIGHT, GreenDecision.UTURN)
            )
