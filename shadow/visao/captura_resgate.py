"""Captura exclusiva da câmera frontal de resgate.
...
def _known_sensor_mode(camera_info):
    ...
    for model_name, mode in config_resgate.RESCUE_KNOWN_SENSOR_MODES.items():
...
class RescueCamera:
    ...
    def __init__(self, camera_index=None):
        ...
        if config_resgate.RESCUE_REQUIRE_TWO_CAMERAS and len(camera_info) < 2:
            raise RuntimeError(
        ...
        if config_resgate.RESCUE_LENS_POSITION is not None:
            try:
                from libcamera import controls
                self.picam2.set_controls({
                    "AfMode": controls.AfModeEnum.Manual,
                    "LensPosition": config_resgate.RESCUE_LENS_POSITION,
                })
    ...
    def _discover_sensor_mode(self):
        return _select_widest_sensor_mode(
            ...
            config_resgate.RESCUE_CAMERA_FPS,
            config_resgate.RESCUE_CAMERA_MAX_WIDTH,
            config_resgate.RESCUE_CAMERA_MAX_HEIGHT,
        )
    ...
    def _prepare_configuration(self, sensor_mode):
        ...
        output_width, output_height = _fit_output_size(
            crop_aspect,
            min(config_resgate.RESCUE_CAMERA_MAX_WIDTH, sensor_width),
            min(config_resgate.RESCUE_CAMERA_MAX_HEIGHT, sensor_height),
        )
        self.output_size = (output_width, output_height)

        mode_fps = float(
            sensor_mode.get("fps", config_resgate.RESCUE_CAMERA_FPS))
        requested_fps = min(config_resgate.RESCUE_CAMERA_FPS, mode_fps)
        ...
        print(
            ...
            f"rotacao 180 graus={'sim' if config_resgate.RESCUE_ROTATE_180 else 'nao'}")
    ...
    def _create_configuration(self, sensor_mode, frame_us):
        ...
        transform = None
        if config_resgate.RESCUE_ROTATE_180:
            try:
