...
def caminho_confiavel_ate_origem(contorno, *, mascara_linha, origem_x):
    """Confirma que existe uma linha continua do robo ate o candidato.

    Alem da trajetoria por bandas, a faixa precisa aparecer no rodape da
    imagem perto da origem esperada. Essa ancora impede o robo de escolher
    retalhos escuros do prata que estao a frente, mas nao possuem caminho
    preto fisicamente conectado ate ele.
    """
    futuro = extrair_ponto_futuro(
        contorno, mascara_linha=mascara_linha, origem_x=origem_x)
    if not futuro.valido or mascara_linha is None or mascara_linha.ndim != 2:
        return False
    if not config.LINE_PATH_REQUIRE_BASE_ANCHOR:
        return True

    height, width = mascara_linha.shape
    base_y = min(height - 1, max(0, int(round(
        height * config.LINE_PATH_BASE_ANCHOR_Y_MIN))))
    silhueta = np.zeros_like(mascara_linha)
    cv2.drawContours(silhueta, [contorno], -1, 255, thickness=cv2.FILLED)
    base = cv2.bitwise_and(mascara_linha[base_y:], silhueta[base_y:])
    if not base.size:
        return False
    intervalos = _intervalos(np.any(base != 0, axis=0))
    if not intervalos:
        return False

    centro, inicio, fim = min(
        ((.5 * (ini + end), ini, end) for ini, end in intervalos),
        key=lambda item: abs(item[0] - origem_x),
    )
    largura = fim - inicio + 1
    return bool(
        abs(centro - origem_x)
        <= width * config.LINE_PATH_BASE_ANCHOR_MAX_OFFSET_RATIO
        and largura <= width * config.LINE_PATH_BASE_ANCHOR_MAX_WIDTH_RATIO
    )
