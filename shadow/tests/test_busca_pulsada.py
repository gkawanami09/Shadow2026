...
class PulsedCycleTests(unittest.TestCase):
    ...
    def test_o_giro_realmente_para_para_observar(self):
        ...
        command = self.search.update(None, now=0.45)
        self.assertEqual(command.angle, 190)
        self.assertEqual(command.speed, 0.0)

    def test_frames_do_giro_nao_sao_aceitos(self):
        command = self.search.update(None, now=0.0)
        self.search.notify_command_written(command.state, now=0.0)
        self.search.update(None, now=0.05)
        self.assertEqual(self.search.state, self.search.ROTATING)
        self.assertFalse(self.search.frame_allowed(0.10))
    ...
    def test_candidato_durante_o_giro_freia_mas_nao_confirma(self):
        ...
        candidato = FakeDetection(
            confirmed=False, track_locked=False, timestamp=0.05)
        freio = self.search.update(candidato, now=0.06)
    ...
    def test_falso_candidato_e_descartado_e_a_busca_retoma(self):
        ...
        candidato = FakeDetection(
            confirmed=False, track_locked=False, timestamp=0.05)
        freio = self.search.update(candidato, now=0.06)
        self.search.notify_command_written(freio.state, now=0.07)

        expirou = 0.07 + cfg.BALL_SEARCH_VERIFY_TIMEOUT_S
...
class PolicyFilterTests(unittest.TestCase):
    def test_politica_pode_recusar_uma_cor(self):
        ...
        search.update(None, now=cfg.BALL_SEARCH_PULSE_S)
        search.notify_command_written(
            search.BRAKE, now=cfg.BALL_SEARCH_PULSE_S)
        assentou = cfg.BALL_SEARCH_PULSE_S + cfg.BALL_SEARCH_SETTLE_S
