...
class AdaptadorEventoTopologicoTests(unittest.TestCase):
    def test_verde_simples_escolhe_o_ramo_futuro_relativo(self):
        esquerdo = BranchObservation(
            BranchKind.LEFT, 90., (-1., 0.), (100., 80.), 2., 11)
        direito = BranchObservation(
            BranchKind.RIGHT, -90., (1., 0.), (340., 80.), 2., 12)
        cena = TopologyObservation(
            decision=TopologyDecision.PENDING,
            junction_image=(224., 180.),
            branches=(esquerdo, direito),
            junction_id=8,
        )

        fundida = _fundir_verde_simples(cena, "right")

        self.assertEqual(fundida.decision, TopologyDecision.RIGHT)
        self.assertEqual(fundida.target_branch.branch_token, 12)

    def test_topologia_sozinha_nao_autoriza_curva_verde(self):
        esquerdo = BranchObservation(
            BranchKind.LEFT, 90., (-1., 0.), (100., 80.), 2., 11)
        cena = TopologyObservation(
            decision=TopologyDecision.LEFT,
            junction_image=(224., 180.),
            branches=(esquerdo,),
            target_branch=esquerdo,
            junction_id=8,
        )

        fundida = _fundir_verde_simples(cena, "straight")

        self.assertEqual(fundida.decision, TopologyDecision.PENDING)
        self.assertIsNone(fundida.target_branch)
