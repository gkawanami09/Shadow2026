#!/usr/bin/env python3
"""Mostra o resultado bruto de ``entrada.onnx`` na câmera de linha.
...
WINDOW = "Shadow2026 - entrada.onnx (q ou Esc fecha)"
...
def main():
    ...
    try:
        while True:
            ...
            preview = frame.copy()
            text = f"ONNX: sem faixa ({elapsed_ms:.0f} ms)"
            color = (0, 0, 255)
            if detection is not None:
                ...
                text = f"ONNX PRATA conf={detection.confidence:.2f} ({elapsed_ms:.0f} ms)"
                color = (0, 255, 0)
